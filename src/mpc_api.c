...
int mpc_api_sum_mta(char* a, char* b, char* alpha, char* beta, char* sum)
{
    ...
    octet A = {EGS_SECP256K1,EGS_SECP256K1,a};
    octet B = {EGS_SECP256K1,EGS_SECP256K1,b};
    octet ALPHA = {EGS_SECP256K1,EGS_SECP256K1,alpha};
    octet BETA = {EGS_SECP256K1,EGS_SECP256K1,beta};
    octet SUM = {0,EGS_SECP256K1,sum};

    rc = MPC_SUM_MTA(&A, &B, &ALPHA, &BETA, &SUM);
    if (rc)
    {
        return rc;
    }
...
#ifdef DEBUG
    printf("mpc_api_sum_mta: A.len %d A.val ", A.len);
    OCT_output(&A);
    printf("mpc_api_sum_mta: B.len %d B.val ", B.len);
    OCT_output(&B);
    printf("mpc_api_sum_mta: ALPHA.len %d ALPHA.val ", ALPHA.len);
    OCT_output(&ALPHA);
    printf("mpc_api_sum_mta: BETA.len %d BETA.val ", BETA.len);
    OCT_output(&BETA);
#endif
