...
def genVector(test_no, k, P=None, Q=None):
    ...
    vector['C0'] = "{}".format(hex(c0)[2:].zfill(k//4))
    vector['C1'] = "{}".format(hex(c1)[2:].zfill(k//4))

    e0, e1 = commitments.bc_setup_challenge(b0, b1, c0, c1, k)
    vector["E0"] = "{}".format(hex(e0)[2:].zfill(k//4))
    vector["E1"] = "{}".format(hex(e1)[2:].zfill(k//4))
    ...
    assert commitments.bc_setup_verify(b0, b1, c0, c1, e0, e1, p0, p1, N), "inconsistent test vector"

    return vector

typeKeys = {
    'commit': [
        'TEST', 'B0', 'B1', 'P', 'Q', 'PQ', 'R0', 'R1', 'C0','C1'
    ],
    'challenge': [
        'TEST', 'B0', 'B1', 'C0', 'C1', 'E0', 'E1'
    ],
    'prove': [
        'TEST', 'R0', 'R1', 'E0', 'E1', 'ALPHA', 'IALPHA', 'PQ', 'P0', 'P1'
    ],
    'verify': [
        'TEST', 'B0', 'B1', 'C0', 'C1', 'E0', 'E1', 'P0', 'P1', 'N' 
    ],
}
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', dest='t', type=str, choices=['commit', 'challenge', 'prove', 'verify'], default='commit',
    ...
    for i in range(nVec):
        ...
        vector = genVector(i, k, P, Q)
        vector = {k: vector[k] for k in keys}

        vectors.append(vector)

    json.dump(vectors, open("ff_{}.json".format(tvType), "w"), indent=2)

    with open("ff_{}.txt".format(tvType), "w") as f:
