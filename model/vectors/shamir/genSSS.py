...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', dest='type', type=str, default='make_shares',
                        help='test vector type', choices=["make_shares", "i_coefficients", "recover"])
    parser.add_argument('--check', action='store_true',
                        help='generate VSS checks')
    parser.add_argument(
        'nVec', type=int, help='number of test vectors to generate')

    args = parser.parse_args()

    print("Generate {} vector(s). Type '{}', with checks: {}".format(
        args.nVec, args.type, args.check))
