...
void ff_2048_cleaned(BIG_1024_58 *a, char *name, int n)
{
    ...
    {
        fprintf(stderr, "FAILURE MTA_ZK_rv_kill. %s was not cleaned\n", name);
        exit(EXIT_FAILURE);
    }
}
...
int main()
{
    ...
    // Run smoke test
    MTA_ZK_commit(&RNG, &pub_key, &pub_mod, &X, &Y, &C1, &rv, &c);
    ...
    rc = MTA_ZK_verify(&priv_key, &priv_mod, &C1, &C2, &c, &ID, &proof);
    if (rc != BIT_COMMITMENT_FAIL)
    {
        printf("FAILURE MTA_ZK error code propagation\n");
        exit(EXIT_FAILURE);
    }
    ...
    MTA_ZK_commitment_fromOctets(&c, &OCT1, &OCT2, &OCT3, &OCT4, &OCT5);

    MTA_ZK_proof_toOctets(&OCT1, &OCT2, &OCT3, &OCT4, &OCT5, &proof);
    MTA_ZK_proof_fromOctets(&proof, &OCT1, &OCT2, &OCT3, &OCT4, &OCT5);

    rc = MTA_ZK_verify(&priv_key, &priv_mod, &C1, &C2, &c, &E, &proof);
    if (rc != MTA_OK)
    {
        printf("FAILURE MTA_ZK octet functions. rc = %d\n", rc);
        exit(EXIT_FAILURE);
    }
